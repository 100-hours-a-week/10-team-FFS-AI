...
class DeserializationError(Exception):
    ...
    def __init__(self, message: str, original_data: bytes):
...
def deserialize(data: bytes, model_class: Type[T]) -> T:
    ...
    try:
        ...
        raise DeserializationError(f"Invalid JSON format: {str(e)}", data)

    except ValidationError as e:

        raise DeserializationError(f"Schema validation failed: {str(e)}", data)
