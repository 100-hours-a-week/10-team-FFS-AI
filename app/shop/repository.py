...
class ShopProductRepository:
    ...
    async def search_by_query(
        ...
        candidates = [
            self._to_candidate(hit) for hit in response.points
        ]
        ...
        return ProductSearchResult(
            category=category, candidates=candidates
        )
    ...
    async def search_multiple(
        ...
        logger.info(
            f"Searching shop products | {log_context}"
            f"query_count={len(queries)}"
        )

        tasks = [
            self.search_by_query(query, parsed, top_k)
        ...
        logger.info(
            f"Shop search completed | {log_context}"
            f"total_candidates={total_found}"
        )
